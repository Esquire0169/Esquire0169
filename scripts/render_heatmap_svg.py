...
Boxes slide in once on a diagonal, then freeze. Regenerated daily by Actions.
...
""".strip()
    ...
    parts = [
        ...
        f'<stop offset="0" stop-color="{HEATMAP_BG2}"/><stop offset="1" stop-color="{HEATMAP_BG}"/>'
        "</linearGradient></defs>",
        f'<rect width="{canvas_w}" height="{canvas_h}" rx="12" fill="url(#hbg)"/>',
        f'<rect x="0.5" y="0.5" width="{canvas_w - 1}" height="{canvas_h - 1}" rx="12" '
        f'fill="none" stroke="{HEATMAP_FRAME}" stroke-width="1" stroke-opacity="0.55"/>',
        f'<line x1="0" y1="{TITLEBAR_H}" x2="{canvas_w}" y2="{TITLEBAR_H}" '
        f'stroke="{HEATMAP_FRAME}" stroke-opacity="0.35"/>',
    ]
    ...
    parts.append(
        ...
        f'stroke="{HEATMAP_FRAME}" stroke-opacity="0.25"/>'
    )
    ...
    parts.append(
        ...
        f'<tspan fill="{MUTED}"> contributions in the last year · контрибуций за год</tspan></text>'
    )
    parts.append(
    ...
    ly += 24
    parts.append(
        f'<text x="{PAD}" y="{ly}" font-size="13" fill="{MUTED}">'
        f'streak / серия <tspan fill="{ACCENT}" font-weight="700">{cs} days</tspan>'
        f'<tspan fill="{MUTED}">   &#183;   longest / макс. </tspan>'
